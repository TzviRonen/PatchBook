...
--
-- One row per (report, GitHub user). The composite primary key is what enforces
-- "one vote per user per report" — a second vote from the same account UPSERTs
...
CREATE TABLE IF NOT EXISTS votes (
  report_id    TEXT    NOT NULL,          -- e.g. _reports/2026-08-29-cve-....md
...
-- The hot query is "all votes for one report" (counts + voter list in one read).
